...
// function 24
void viewScoreOfStudent(vector<Class> myClass, int posClass, int posCourse, int posStudent)
{
	gotoXY(40, 6);
	cout << myClass[posClass].cou[posCourse].stu[posStudent].lastName << " " << myClass[posClass].cou[posCourse].stu[posStudent].firstName << "\t";
	cout << "Mid: " << myClass[posClass].cou[posCourse].Sb[posStudent].Mid << "\t";
	cout << "Final: " << myClass[posClass].cou[posCourse].Sb[posStudent].Final << "\t";
	cout << "Lab: " << myClass[posClass].cou[posCourse].Sb[posStudent].Lab << "\t";
	cout << "Bonus: " << myClass[posClass].cou[posCourse].Sb[posStudent].Bonus << "\n";
}

// function 26
void viewAttendanceOfStudent(vector<Class> myClass, int posClass, int posCourse, int posStudent)
{
	if (myClass[posClass].cou[posCourse].stu[posStudent].Checkin == true)
	{
		gotoXY(40, 6);
		cout << myClass[posClass].cou[posCourse].stu[posStudent].lastName << " " << myClass[posClass].cou[posCourse].stu[posStudent].firstName << endl;
	}
	else cout << "absent";
}
...
void ExportScoreToFile(const char* path, vector<Class> myClass, int posClass, int posCourse)
{
	ofstream fout;
	fout.open(path, ios::out);
	if (!fout)
	...
	fout << "No, ID, Mid, Final, Lab, Bonus" << endl;
	for (int i = 0; i < myClass[posClass].cou[posCourse].Sb.size(); i++)
	{
		fout << i+1 << ","
			 << myClass[posClass].cou[posCourse].Sb[i].ID 
			 << "," << myClass[posClass].cou[posCourse].Sb[i].Mid 
			 << "," << myClass[posClass].cou[posCourse].Sb[i].Final
			 << "," << myClass[posClass].cou[posCourse].Sb[i].Lab
			 << "," << myClass[posClass].cou[posCourse].Sb[i].Bonus << endl;
	}
	fout.close();
}
...
void ExportAttendance(const char* path, vector<Class> myClass, int posClass, int posCourse)
{
	ofstream fout;
	fout.open(path, ios::out);
	if (!fout)
	...
	fout << "No, ID, Last name, First name, Gender, DayOfBirth" << endl;
	for (int i = 0; i < myClass[posClass].cou[posCourse].stu.size(); i++)
	{
		if (myClass[posClass].cou[posCourse].stu[i].Checkin == true)
		{
			gotoXY(40, 6 + i);
			fout << myClass[posClass].cou[posCourse].stu[i].ID
				 << "," << myClass[posClass].cou[posCourse].stu[i].lastName 
				 << "," << myClass[posClass].cou[posCourse].stu[i].firstName 
				 << "," << myClass[posClass].cou[posCourse].stu[i].gender
				 << "," << myClass[posClass].cou[posCourse].stu[i].doB.year
				 << "-" << myClass[posClass].cou[posCourse].stu[i].doB.month
				 << "-" << myClass[posClass].cou[posCourse].stu[i].doB.day
				 << endl;
		}
	}
}
...
// function 36
void checkin_Ofcourse(vector<Class> &myClass, int &posClass, int &posCourse, int &posStudent,User &userLogin)
{
	//if (stoi(userLogin.ID) == myClass[posClass].cou[posCourse].stu[posStudent].ID)
	if (myClass[posClass].cou[posCourse].stu[posStudent].Checkin == true)
	{
		gotoXY(40, 5);
		cout << myClass[posClass].cou[posCourse].CourseName << ": Sucess";
	}
	else
	{
		gotoXY(40, 5);
		cout << myClass[posClass].cou[posCourse].CourseName << ": Fail";
	}
}

//function 38
void MyScore(vector<Class> &myClass, int &posClass, int &posCourse, int &posStudent, User &userLogin)
{
	gotoXY(40, 5);
	cout << "Mid: " << myClass[posClass].cou[posCourse].Sb[posStudent].Mid << "\t";
	cout << "Final: " << myClass[posClass].cou[posCourse].Sb[posStudent].Final << "\t";
...
void editGrade(vector<Class> &myClass, int posClass, int posCourse, User &userLogin)
{
	Scoreboard temp;
	int k = 5;
	...
	for (int i = 0; i < myClass[posClass].cou[posCourse].Sb.size(); i++)
	{
		if (myClass[posClass].cou[posCourse].Sb[i].ID == temp.ID)
		{
			gotoXY(40, ++k);
			cout << "Mid: ";
			cin >> myClass[posClass].cou[posCourse].Sb[i].Mid;
			gotoXY(40, ++k);
			cout << "Final: ";
			cin >> myClass[posClass].cou[posCourse].Sb[i].Final;
			gotoXY(40, ++k);
			cout << "Lab: ";
			cin >> myClass[posClass].cou[posCourse].Sb[i].Lab;
			gotoXY(40, ++k);
			cout << "Bonus: ";
			cin >> myClass[posClass].cou[posCourse].Sb[i].Bonus;
			break;
		}
	}
}

void editAttendance(vector<Class> &myClass, int posClass, int posCourse, User &userLogin)
{
	char temp;
	int tmp;
	gotoXY(40, 5);
	...
	cin >> tmp;
	gotoXY(40, 6);
	cout << "Absent or not(Y/N): ";
	cin >> temp;
	for (int i = 0; i < myClass[posClass].cou[posCourse].stu.size(); i++)
	{
		if (myClass[posClass].cou[posCourse].stu[i].ID == tmp)
		{
			if (temp == 'Y' || temp == 'y')
				myClass[posClass].cou[posCourse].stu[i].Checkin = false;
